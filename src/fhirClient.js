// 與 FHIR 伺服器（HAPI baseR4）溝通的薄封裝層。
// 使用 Node 18+ 內建的全域 fetch，無額外相依套件。
//
// 重點：
//  - 統一帶上 application/fhir+json 標頭
//  - 建立/更新時自動貼上租戶 meta.tag（命名空間隔離）
//  - 搜尋一律加上 _tag 過濾，只取得本園資料
...
async function request(method, path, body) {
  ...
  if (!res.ok) {
    ...
    const e = new Error(`FHIR ${method} ${path} 失敗 (HTTP ${res.status})${detail ? '：' + detail : ''}`);
    e.status = res.status;
...
export const fhir = {
  ...
  /**
   * 搜尋資源；自動加上租戶 _tag 過濾。
   * params: 物件，例如 { 'participant': 'Practitioner/1', '_count': 50 }
   * 回傳 entry 陣列（已展開 resource）。
   */
  ...
  async searchAll(resourceType, params = {}, pageLimit = 20) {
    const qs = new URLSearchParams();
    qs.set('_tag', `${SYSTEMS.tenant}|${config.tenantTag}`);
    qs.set('_count', String(params._count || 100));
    for (const [k, v] of Object.entries(params)) {
